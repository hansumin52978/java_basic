...
//	} (x) 반환 유형 (return type)은 오버로딩에 영향을 미치지 못합니다.
	...
	public int calcRectArea(int r) {
		return r * r;
//		Math.pow(2, 8); // 2의 8승이란는 뜻 2*2*2*2*2*2*2*2
	}
	
	public int calcRectArea(int width, int height) {
		return width * height;
	}
	
	public double calcRectArea(int ceil, int floor, int height) {
