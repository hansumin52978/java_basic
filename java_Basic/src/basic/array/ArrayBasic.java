...
//		arr[5] = 70; (x) index 범위를 초과
		...
		// 인덱스는 0부터 시작하기 때문에, 마지막 인덱스는 항상 배열의 길이보다 하나작다.
		// 길이가 5인 배열은 마지막 인덱스가 4이다.
		// 그런데 arr[5]라고 작성해도 빨간 줄이 그어지지 않는다.
		// 문법 상으로는 문제가 없지만, 실제 메모리의 배열에는 5번 인덱스가 없다.
		// 오로지 개발자의 선행 지식이 있어야 예방할 수 있는 에러이므로 조심!
		
		// 4. 배열의 총 크기(길이) 확인. -> 배열변수명.length
		System.out.println("arr 배열의 길이: " + arr.length); //5
		...
		// 7. 배열 내부의 요소값을 문자열 형태로 한 눈에 출력하기
//		System.out.println(arr); // 16진수의 알 수 없는 주소 값이 나온다. ( 그 이유는 배열은 객체 타입이기 때문이다. )
